...
```{r, include = FALSE}
have_packages = all(sapply( c("ggplot2","phylopath","phylolm","ape","tmbstan"), FUN=requireNamespace))
...
We note in passing that we could instead estimate Bayesian credible intervals, using improper (flat) priors on all input parameters and using `tmbstan` to automate MCMC sampling.  To speed up the demonstration, we eliminate the tree transformations and only sample path coefficients:
