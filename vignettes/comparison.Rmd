...
First, we specifically explore a Poisson-distributed PGLM, comparing `phylosem` against `phylolm::phyloglm` (which uses Generalized Estimating Equations), `phyr::pglmm_compare` (which uses maximum likelihood), and `brms` (which fits a Bayesian hierarchical model).

```{r, echo=TRUE, message=FALSE, fig.width=6, fig.height=6, out.width = "75%"}
# Settings
Ntree = 100
sd_x = 0.3
sd_y = 0.3
...
pdat$Method <- rep(c('phylolm::phyloglm', 'phyr::pglmm_compare', 'brms::brm', 'phylosem::phylosem'), each = 2)
ggplot(pdat, aes(
  x = Estimate, xmin = Estimate - StdErr, xmax = Estimate + StdErr, y = Param, color = Method
)) +
  geom_pointrange(position = position_dodge(width = 0.6)) +
  theme_classic() + theme(panel.grid.major.x = element_line(), panel.grid.minor.x = element_line())
```
