...
We next compare with a single run of `phylopath`.  This again confirms that runtimes are comparable and results are identical for standardized or unstandardized coefficients.
...
We next compare syntax and runtime against R-package `sem`.  This confirms that runtimes are comparable when specifying a star-phylogeny in `phylosem` to match the assumed structure in `sem`
