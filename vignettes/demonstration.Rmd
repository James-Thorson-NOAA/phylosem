...
# use render( "demonstration.Rmd", pdf_document()) to build PDF, and perhaps tinytex::latexmk() to install dependencies
```
...
We can also extract the standard errors, calculated as the square-root of the diagonal elements of the covariance matrix.  Plotting estimates plus or minus 1.96 times the standard errors gives an approximate 95% confidence interval:

```{r, eval=have_packages, echo=TRUE, message=FALSE, fig.width=4, fig.height=4}
library(ggplot2)
...
ggplot(pdat, aes( x=Param, y = Estimate, ymin = Estimate - 1.96*StdErr, ymax = Estimate + 1.96*StdErr)) +
